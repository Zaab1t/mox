...
def update_lora(selected_facet=None):
    ...
    if selected_facet is not None:
        klasse_list = g.helper.read_klasse_list(selected_facet)
        g.klasse_info = g.helper.basic_klasse_info(klasse_list)

        hovedgrupper = []
        for klasse in g.klasse_info:
